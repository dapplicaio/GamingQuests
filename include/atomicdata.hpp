...
inline namespace atomicdata {
    ...
    inline std::vector <uint8_t> toVarintBytes(uint64_t number, uint64_t original_bytes = 8) {
        if (original_bytes < 8) {
    ...
    inline uint64_t unsignedFromVarintBytes(std::vector <const uint8_t>::iterator &itr) {
        uint64_t number = 0;
    ...
    inline std::vector <uint8_t> toIntBytes(uint64_t number, uint64_t byte_amount) {
        std::vector <uint8_t> bytes = {};
    ...
    inline uint64_t unsignedFromIntBytes(std::vector <const uint8_t>::iterator &itr, uint64_t original_bytes = 8) {
        uint64_t number = 0;
        uint64_t multiplier = 1;
    ...
    inline std::vector <uint8_t> serialize_attribute(const std::string &type, const ATOMIC_ATTRIBUTE &attr) {
    ...
    inline ATOMIC_ATTRIBUTE deserialize_attribute(const std::string &type, std::vector <const uint8_t>::iterator &itr) {
        if (type.find("[]", type.length() - 2) == type.length() - 2) {
            //Type is an array
    ...
    inline std::vector <uint8_t> serialize(ATTRIBUTE_MAP attr_map, const std::vector <FORMAT> &format_lines) {
        uint64_t number = 0;
        std::vector <uint8_t> serialized_data = {};
    ...
    inline ATTRIBUTE_MAP deserialize(const std::vector <uint8_t> &data, const std::vector <FORMAT> &format_lines) {
